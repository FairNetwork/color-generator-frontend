...
export function ColorCarousel({
  ...
  const instructionText = isMobile ? "Tap the color area to save it" : "Press the spacebar to save"
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <div className="relative z-10 max-w-xl space-y-3 px-4 pb-24 text-center sm:px-6">
          <div className="font-mono text-4xl font-bold text-white drop-shadow-lg sm:text-5xl md:text-7xl">{color}</div>
          <div className="text-xs font-medium text-white/90 drop-shadow sm:text-sm">{instructionText}</div>
        </div>
        ...
        <div className="absolute inset-x-0 bottom-0 z-10 px-4 pb-4 sm:px-6 sm:pb-6">
          <div className="rounded-2xl bg-black/35 p-3 shadow-lg backdrop-blur sm:p-4">
            <div className="flex items-center justify-between text-[11px] font-medium text-white/80 sm:text-xs">
              ...
              <span>{isPaused ? "Paused" : `${remainingSeconds.toFixed(1)}s`}</span>
            </div>
            <Progress value={progress} className="mt-2 h-1.5 bg-white/30 sm:h-2" />
          </div>
        </div>
      </div>
    </div>
